...
async function doVote() {
  ...
  for (const p of alive) {
    const hunt = isDark(p.role) ? "" : "\nОХОТА: злодеям вычислять некого — они знают своих, потому днём ленивы: голосуют наобум, отмахиваются, болтают невпопад и не по делу. Твой козырь — усердие: думай, спорь по существу, следи за поведением. Красные флаги злодея — пустые доводы, вялость, безразличие к казни и настроение не в лад со столом.";
  let r;
    if (p.human) {
